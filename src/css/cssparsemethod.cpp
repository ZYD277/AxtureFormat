...
bool CssParseMethod::parseFile(RTextFile *file)
{
    ...
    QDir dir(QFileInfo(file->fileName()).path());

    QStringList nameFilters;
    nameFilters<<"*.css";

    QFileInfoList fileInfoList = dir.entryInfoList(nameFilters,QDir::Files | QDir::NoDotAndDotDot);

    QString cssDataStr;

    for(QFileInfo & fileInfo:fileInfoList){
        QFile cssFile(fileInfo.filePath());
